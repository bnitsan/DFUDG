%%
function getLumRadVsMassSims3TwoCol(MassBins,sims,titleFig,nS,rH,CutRadius,saveFlag,saveName,Vertical)
...
if Vertical
    figure('Renderer', 'opengl', 'Position', [1200 100 450 900])
    subplot(2,1,1)
else
    figure('Renderer', 'opengl', 'Position', [1200 100 900 400])
    subplot(1,2,1)
end
...
% compute total merged GCs per batch
TotalMerged=0;
for k = 1:length(sims)
    TotalMerged = TotalMerged + sims{k}.N-sims{k}.Ncurrent;
end
AvgMergers=TotalMerged/length(sims);

SimsNumPerBin(SimsNumPerBin==0)=-1; % move the data point at zero to negative side
...
scatter(DatAvgMass,DatNumPerBin,120,'filled');
hold on;
scatter(AvMassIC,AvNGCIC,45,[0.9290, 0.6940, 0.1250],'filled')
heb=errorbar(SimsAvgMass,SimsNumPerBin/length(sims)/3,min(STDN,SimsNumPerBin/length(sims)/3-0.1),STDN,'.','CapSize',18,'MarkerSize',12,'Color','blue');
%heb.LineWidth = 2;

set(gca,'FontSize',15);
xlabel('M bins [$10^5~M_\odot$]','interpreter','latex');
ylabel('$\left<{\rm Num.~ of~ GCs}\right>$','interpreter','latex')
grid on;
if MassBins(1)>=0.6
    axis([1 max(max(SimsAvgMass),max(DatAvgMass))*1.1 0 max(max(max(SimsNumPerBin/length(sims)),max(DatNumPerBin)),max(AvNGCIC))*1.2])
else
    axis([0.4 max(max(SimsAvgMass),max(DatAvgMass))*1.1 0 max(max(max(SimsNumPerBin/length(sims)),max(DatNumPerBin)),max(AvNGCIC))*1.2])    
end
    %axis([0 27 0 15])
legend('Data','Initial conditions','Simulation','Location','SouthWest')
title(['Average number of mergers: ' num2str(AvgMergers,3)],'interpreter','latex')
set(gca,'XScale','log')
set(gca,'YScale','log')
